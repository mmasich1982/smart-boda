...
async function ensureSyncQueueStore() {
  try {
    ...
    const testRecord = await db.queryRows(SYNC_QUEUE_STORE);
...
export const getQueuedRecords = async () => {
  try {
    const records = await db.queryRows(SYNC_QUEUE_STORE);
...
export const addToSyncQueue = async (record) => {
  try {
    ...
    await db.insertRow(SYNC_QUEUE_STORE, newRecord);
...
export const removeFromSyncQueue = async (recordId) => {
  try {
    await db.deleteRow(SYNC_QUEUE_STORE, recordId);
...
export const updateQueuedRecord = async (recordId, updates) => {
  try {
    const updated = await db.updateRow(SYNC_QUEUE_STORE, recordId, updates);
    if (updated) {
...
export const updateLastSyncTime = async () => {
  try {
    ...
    await db.kvSet(LAST_SYNC_TIME_KEY, now);
...
export const hoursSinceLastSync = async () => {
  try {
    const lastSyncStr = await db.kvGet(LAST_SYNC_TIME_KEY);
...
export const clearSyncQueue = async () => {
  try {
    ...
    for (const record of records) {
      await db.deleteRow(SYNC_QUEUE_STORE, record.id);
    }
...
export const getSyncStats = async () => {
  try {
    const queued = await getQueuedRecords();
    const lastSync = await db.kvGet(LAST_SYNC_TIME_KEY);
