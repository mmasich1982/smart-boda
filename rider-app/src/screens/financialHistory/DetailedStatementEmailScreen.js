...
/**
 * DetailedStatementEmailScreen.js - INDEXEDDB MIGRATION v2.0
 * ✅ MIGRATION: Complete IndexedDB integration
 * ✅ INITIALIZATION: Proper isMounted checks to prevent state updates after unmount
...
export default function DetailedStatementEmailScreen({ navigation, route }) {
  ...
  useEffect(() => {
    return () => {
      ...
      console.log('[DetailedStatementEmailScreen] Component unmounted');
    };
  }, []);
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      console.log(`[DetailedStatementEmailScreen] Proceeding to confirmation with email: ${email.trim()}`);

      // Navigate to confirmation screen with all required params
      if (isMountedRef.current) {
      ...
      console.error('[DetailedStatementEmailScreen] Submit email error:', err);
      if (isMountedRef.current) {
