...
export default function RevenueTargetsScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.get(`/financial/targets?rider_id=${effectiveRiderId}`).catch(() => ({ data: {} })),
      api.get(`/financial/net-profit?rider_id=${effectiveRiderId}&period=today`).catch(() => ({ data: {} })),
      api.get(`/financial/net-profit?rider_id=${effectiveRiderId}&period=this_week`).catch(() => ({ data: {} })),
      api.get(`/financial/net-profit?rider_id=${effectiveRiderId}&period=this_month`).catch(() => ({ data: {} })),
    ])
  ...
  const handleRemoveTarget = async () => {
    try {
      await api.delete(`/financial/targets?rider_id=${effectiveRiderId}&period=${activePeriod}`);
