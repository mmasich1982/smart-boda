...
// Issue 15 fix: Terms of Service content is now fetched from backend, not hardcoded
    ...
    <View style={styles.container}>
      ...
      {error && (
        <View style={styles.errorBox}>
          <Text style={styles.errorText}>⚠️ Could not load Terms of Service</Text>
          <Text style={styles.errorSubtext}>{error}</Text>
        </View>
      )}
      
      {!loading && documents.termsOfService && (
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          <Text style={styles.contentText}>{documents.termsOfService}</Text>
